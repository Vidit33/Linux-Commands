...
2. cp export_chat_hook.py ~/.claude/hooks/export_chat_hook.py

3. Edit ~/.claude/settings.json and merge this hooks block in (don't wipe existing keys). If the file doesn't exist yet, create it with just this:
  {
    "hooks": {
      "Stop": [
        {
...
4. python3 -c "import json; json.load(open('$HOME/.claude/settings.json')); print('valid')"

5. Hooks load at session startup, so new windows pick it up automatically.
